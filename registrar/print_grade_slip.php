<?php
/**
 * Registrar - Print Student Grade Slip
...
if ($studentId <= 0) {
    redirectWithMessage('students.php', 'Student ID must be provided.', 'danger');
}
...
// 1. Get the context (Semester, School Year) explicitly or from latest enrollment
$targetSemester = $_GET['semester'] ?? '';
$targetSY = $_GET['school_year'] ?? ($_GET['year'] ?? '');

if (empty($targetSemester) || empty($targetSY)) {
    // Fall back to latest enrollment semester/year for this student
    $latestStmt = $conn->prepare("
        SELECT cs.semester, cs.school_year 
        FROM enrollments e 
    ...
    if (!$latest) {
        redirectWithMessage('students.php', 'No enrollments found to print a grade slip for.', 'warning');
    }
    
    $targetSemester = $latest['semester'];
    $targetSY = $latest['school_year'];
}
...
if ($userRole === 'registrar_staff') {
    ...
    if ($student['dept_id'] != $staffDeptId) {
        redirectWithMessage('students.php', 'Unauthorized: You do not have access to this student\'s grade slip.', 'danger');
    }
}
...
while ($row = $gradesRes->fetch_assoc()) {
    ...
    if (!empty($row['instructor_name']) && !in_array($row['instructor_name'], $instructorNames)) {
        $instructorNames[] = $row['instructor_name'];
    }
}
    ...
    <table class="signatures">
        <tr>
            <td>
                ...
                <div class="sig-box">
                    <div class="sig-line" style="text-transform: capitalize; font-size: 12pt;">
                        <?php echo !empty($instructorNames) ? htmlspecialchars(implode(' / ', $instructorNames)) : 'Multiple Instructors'; ?>
                    </div>
                    <div class="sig-title">Instructor(s)</div>
                </div>
            </td>
            <td>
                <div style="font-family: 'Times New Roman', Times, serif; font-size: 11pt; margin-bottom: 30px;">Noted by:</div>
                <div class="sig-box">
                    <div class="sig-line" style="font-size: 12pt;"><?php echo htmlspecialchars($registrarName); ?></div>
                    <div class="sig-title"><?php echo htmlspecialchars($registrarPosition); ?></div>
                </div>
            </td>
        </tr>
    </table>
